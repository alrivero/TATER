...
def process_video(root_directory, video_title, transcript_title, temp_dir, log_file_path, gpu_ID, process_ID, lock, gpu_queue, include_audio=True):
    ...
    transcript_path = os.path.join(root_directory, "transcripts", transcript_title)
    video_paths = os.listdir(os.path.join(root_directory, "videos"))
    ...
    for i, segment in enumerate(tqdm(video_segments, desc=f"Processing {video_title}", position=0)):
        try:
            ...
            if len(video_paths) == 1:  # Single video
                ...
                if include_audio:
                    audio_segment, sample_rate = extract_audio_from_video(
                        os.path.join(root_directory, "videos", video_paths[0]),
                        segment["start"],
                        segment["end"],
                        bad_frames,
                        vid_fps
            ...
            if segment["text"] != "":
                segment_group.attrs["og_id"] = segment["id"] - silent_offset
            else:
                silent_offset += 1
...
def save_to_hdf5(root_directory, hdf5_path, temp_dir, log_file_path, gpus, tasks_per_gpu, include_audio):
    ...
    transcript_titles = sorted(os.listdir(os.path.join(root_directory, "transcripts")))
    video_titles = sorted(os.listdir(os.path.join(root_directory, "videos")))
