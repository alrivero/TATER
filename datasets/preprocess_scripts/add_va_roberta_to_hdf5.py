...
def define_segments(transcripts_df, va_df, roberta_df, roberta_red_df, add_roberta_va=True):
    new_segments = {}
    ...
    for _, row in transcripts_df.iterrows():  # Use iterrows to iterate over rows
        ...
        roberta_rows = roberta_df[roberta_df["index"] == row["index"]]
        roberta_rows = roberta_rows.sort_values(by='feat').reset_index(drop=True)
        roberta_red_rows = roberta_red_df[roberta_red_df["index"] == row["index"]]
        roberta_red_rows = roberta_red_rows.sort_values(by='feat').reset_index(drop=True)

        segment_dict["valence_arousal"] = torch.tensor(va_rows['value'].values, dtype=torch.float32)
        segment_dict["roberta"] = torch.tensor(roberta_rows['value'].values, dtype=torch.float32)
        segment_dict["roberta_pca_5"] = torch.tensor(roberta_red_rows['value'].values, dtype=torch.float32)
...
def process_video(root_directory, video_title, transcript_df, va_df, roberta_df, roberta_red_df, out_dir, log_file_path, gpu_ID, process_ID, lock, gpu_queue, include_audio=True):
    ...
    with h5py.File(temp_hdf5_path, 'r+') as temp_hdf5_file:
        ...
        video_segments = define_segments(transcript_df, va_df, roberta_df, roberta_red_df)
        ...
        for group_id in temp_hdf5_file.keys():
            try:
                ...
                # Create or update 'roberta' dataset
                if "roberta" not in group_data:
                    group_data.create_dataset("roberta", data=video_segments[og_id]["roberta"])
                else:
                    group_data["roberta"][()] = video_segments[og_id]["roberta"]

                # Create or update 'roberta_pca_5' dataset
                if "roberta_pca_5" not in group_data:
                    group_data.create_dataset("roberta_pca_5", data=video_segments[og_id]["roberta_pca_5"])
                else:
                    group_data["roberta_pca_5"][()] = video_segments[og_id]["roberta_pca_5"]
...
def save_to_hdf5(root_directory, out_dir, transcript_file, va_file, roberta_file, roberta_red_file, log_file_path, gpus, tasks_per_gpu, include_audio):
    active_processes = []
    ...
    all_roberta_df = pd.read_csv(roberta_file)
    all_roberta_red_df = pd.read_csv(roberta_red_file)
    ...
    all_roberta_df[['PID', 'speaker_id', 'index']] = all_roberta_df['group_id'].str.split('_', expand=True)
    all_roberta_red_df[['PID', 'speaker_id', 'index']] = all_roberta_red_df['group_id'].str.split('_', expand=True)
    ...
    for video_title, PID in zipped_titles:
        ...
        roberta_df = all_roberta_df[all_roberta_df["PID"] == PID]
        roberta_red_df = all_roberta_red_df[all_roberta_red_df["PID"] == PID]
        ...
        video_encoding_proc = Process(target=process_video, args=(root_directory, video_title, transcript_df, va_df, roberta_df, roberta_red_df, out_dir, log_file_path, available_gpu, prev_process, lock, gpu_queue, include_audio))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--roberta_file', type=str, required=True, help='File with roberta data.')
    parser.add_argument('--roberta_red_file', type=str, required=True, help='File with dim-reduced roberta data.')
    ...
    roberta_file = args.roberta_file
    roberta_red_file = args.roberta_red_file
    log_file_path = args.log_file
    gpus = [int(x) for x in args.gpus.split(" ")]
    tasks_per_gpu = [int(x) for x in args.tasks_per_gpu.split(" ")]
    include_audio = not args.exclude_audio

    save_to_hdf5(input_dir, out_dir, transcript_file, va_file, roberta_file, roberta_red_file, log_file_path, gpus, tasks_per_gpu, include_audio)
