...
def define_segments(transcript_path):
    all_segments = []
...
def extract_segment_data_split_video(segment, root_directory, video_paths, fan_face_detector, fan_landmark_detector, mp_landmark_detector, split_min_len=20):
    ...
    cap = cv2.VideoCapture(os.path.join(root_directory, "video", video_paths[-1]))
    ...
    if int(start // video_len) == int(end // video_len):
        cap = cv2.VideoCapture(os.path.join(root_directory, "video", video_paths[int(start // video_len)]))
        ...
        # Process first part
        cap1 = cv2.VideoCapture(os.path.join(root_directory, "video", video_paths[int(start // video_len)]))
        ...
        # Process second part
        cap2 = cv2.VideoCapture(os.path.join(root_directory, "video", video_paths[int((start // video_len) + 1)]))
...
def extract_segment_data_single_video(segment, root_directory, video_path, fan_face_detector, fan_landmark_detector, mp_landmark_detector):
    # Seek video based on segment duration and gather frames
    cap = cv2.VideoCapture(os.path.join(root_directory, "video", video_path))
    fps = cap.get(cv2.CAP_PROP_FPS)
    start = int(fps * segment["start"])
    end = int(fps * segment["end"])
...
def extract_audio_from_video(video_path, segment_start, segment_end, bad_frames, vid_fps):
    ...
    with VideoFileClip(video_path) as video_clip:
        ...
        start = int(sample_rate * segment_start)
        end = int(sample_rate * segment_end)
...
def process_video(root_directory, video_title, transcript_title, temp_dir, log_file_path, gpu_ID, process_ID, lock, gpu_queue, include_audio=True):
    start_time = time.time()  # Start timing

    # Gather the correct transcript and possible video
    transcript_path = os.path.join("/home/arivero/iHiTOP_Remaining", "transcripts", transcript_title)
    video_paths = os.listdir(os.path.join(root_directory, "video"))
    ...
    try:
        ...
        print(f"Problem creating HDF5 file for {video_title}!")
        print(e)
        gpu_queue.put((gpu_ID, process_ID))
        return None

    # Next, define all segments present in video
    video_segments = define_segments(transcript_path)
    ...
    for i, segment in enumerate(tqdm(video_segments, desc=f"Processing {video_title}", position=0)):
        try:
            # Video processing
            if len(video_paths) == 1:  # Single video
                ...
                if include_audio:
                    audio_segment, sample_rate = extract_audio_from_video(
                        os.path.join(root_directory, "video", video_paths[0]),
                        segment["start"],
                        segment["end"],
                        bad_frames,
                        vid_fps
            ...
            segment_group = temp_hdf5_file.create_group(str(i))
...
def save_to_hdf5(root_directory, hdf5_path, temp_dir, log_file_path, gpus, tasks_per_gpu, include_audio):
    active_processes = []
    ...
    transcript_titles = sorted(os.listdir(os.path.join("/home/arivero/iHiTOP_Remaining", "transcripts")))
    video_titles = sorted(os.listdir(os.path.join(root_directory, "video")))
    ...
    zipped_titles = [(video_dict[base], transcript_dict[base]) for base in common_basenames]

    # Print the result
    for transcript, video in zipped_titles:
        print(f"Transcript: {transcript}, Video: {video}")
    ...
    # i = 0
    # process_video(root_directory, video_titles[i], transcript_titles[i], temp_dir, log_file_path, 0, 0, lock, gpu_queue)
    # process_video(root_directory, video_titles[1], transcript_titles[1], temp_dir, log_file_path, lock)

    # with Pool(processes=max_processes) as pool:
    #     results = []
    #     for video_title, transcript_title in zip(video_titles, transcript_titles):
    #         result = pool.apply_async(process_video, (root_directory, video_title, transcript_title, temp_dir, log_file_path, lock), callback=lambda _: progress.update())
    ...
    for video_title, transcript_title in zipped_titles:
        ...
        video_encoding_proc = Process(target=process_video, args=(root_directory, video_title, transcript_title, temp_dir, log_file_path, available_gpu, prev_process, lock, gpu_queue, include_audio))
...
if __name__ == "__main__":
    ...
    save_to_hdf5(input_dir, output_file, temp_dir, log_file_path, gpus, tasks_per_gpu, include_audio)
