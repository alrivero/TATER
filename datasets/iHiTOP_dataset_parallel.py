...
class iHiTOPDatasetParallel(BaseVideoDataset):
    ...
    def _get_hdf5_file(self, file_idx):
        ...
        if self.worker_hdf5_handles is None or file_idx not in self.worker_hdf5_handles:
            raise RuntimeError(f"HDF5 file for index {file_idx} not loaded in this worker.")
    ...
    def initialize_worker_hdf5_handles(self):
        ...
        print(f"Worker {torch.utils.data.get_worker_info().id} initialized HDF5 handles.")
    ...
    def __getitem_aux__(self, index):
        ...
        video_group = self._get_hdf5_file(v)[str(s)]
        video_dict = {}
...
def get_datasets_iHiTOP_parallel(config=None):
    if get_rank() == 0:
        ...
        if not os.path.exists(config.dataset.iHiTOP.data_idxs):
            ...
            effective_seg_count, data_idxs = cull_indices(hdf5_file_paths, seg_indices, config)
            np.save(config.dataset.iHiTOP.data_idxs, data_idxs)
            np.save(config.dataset.iHiTOP.all_seg_idxs, seg_indices)
            np.save(config.dataset.iHiTOP.effective_seg_count, effective_seg_count)

        if not os.path.exists(config.dataset.iHiTOP.final_idxs):
            segment_idxs = np.load(config.dataset.iHiTOP.all_seg_idxs)  # Ensure seg_indices is consistent
            data_idxs = np.load(config.dataset.iHiTOP.data_idxs)  # Ensure seg_indices is consistent

            random_idxs = segment_idxs[data_idxs]
